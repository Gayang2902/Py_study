...
# 적 enemy 캐릭터
enemy = pygame.image.load("/Users/kd_mb/Desktop/code/Python/활용/Game/enemy.png")
...
while running:
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYUP: # 키가 떼지는걸 확인
            if event.key == pygame.K_LEFT or event.key == pygame.K_RIGHT:
                to_x = 0
            elif event.key == pygame.K_UP or event.key == pygame.K_DOWN:
                to_y = 0

    character_x_pos += to_x * dt # 프레임정보를 곱해서 프레임에 따른 이동속도를 보정해줘야 함
    ...
    # 충돌 처리를 위한 rect정보를 업데이트 : 기본 rect정보는 초기위치에 있으므로..)
    ...
    screen.blit(background, (0, 0)) # 배경 그리기 / 좌표상에서 (0, 0)은 왼쪽위이다.
    ...
    pygame.display.update() # 게임화면을 다시 그리기! (업데이트) 

# 잠시 대기
pygame.time.delay(2000) # 2초 정도 대기(2ms * 1000) = (2s)
